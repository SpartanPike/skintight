...
const Testform = ({navigation, route}) => {
  ...
  return (
    <View
      style={tw`h-full w-full bg-indigo-600 flex justify-between items-center`}>
      <View
        style={tw`w-full h-20 flex items-center justify-center bg-white rounded-b-full`}>
      ...
      {images ? (
        <Text>imgs selected {images.length} </Text>
      ) : (
        <Text>no img </Text>
      )}

      <View style={tw`w-full p-10 flex gap-4  items-center`}>
        <TextInput
          placeholder="Write descreiption about your disease"
          autoCapitalize="none"
          multiline
          numberOfLines={8}
          onChangeText={Text => setDescription(Text)}
          maxLength={maxDesLen}
          style={tw`p-4 h-48 w-full bg-white border border-indigo-200  rounded-xl `}
        />

        <TouchableOpacity
          onPress={submitDes}
          style={tw`px-5 h-12 bg-white rounded-full bg-orange-400 flex justify-center items-center`}>
