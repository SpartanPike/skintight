...
const Signup = ({navigation}) => {
  ...
  return (
    <View style={tw`h-full bg-indigo-50`}>
      <View
        style={tw`w-full h-[12%] flex justify-center bg-indigo-500 rounded-b-full`}>
        <Text style={tw` text-center text-3xl font-light text-white`}>
          skinsight
        </Text>
      </View>

      <View style={tw`h-88% p-5 flex justify-between items-center`}>
        <View style={tw`w-full flex items-center justify-evenly h-[90%]`}>
          <Image
            source={require('../public/snail.gif')}
            style={tw`h-24 w-24`}
          />
          <Text style={tw` text-center text-base font-light text-indigo-900`}>
          ...
          <TouchableOpacity
            onPress={signup}
            style={tw`px-5 h-12 bg-white w-[50%] rounded-full bg-orange-400 flex justify-center items-center`}>
